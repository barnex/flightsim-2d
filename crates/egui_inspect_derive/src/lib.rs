...
use syn::{parse_macro_input, parse_quote, Data, DeriveInput, Field, Fields, FieldsNamed, GenericParam, Generics, Index};
...
fn add_trait_bounds(mut generics: Generics) -> Generics {
	for param in &mut generics.params {
		if let GenericParam::Type(ref mut type_param) = *param {
			type_param.bounds.push(parse_quote!(egui_inspect::EguiInspect));
		}
	}
	generics
}
...
fn handle_custom_func(field: &Field, mutable: bool, attrs: &AttributeArgs) -> Option<TokenStream> {
	...
	if mutable && !attrs.no_edit && attrs.custom_func_mut.is_some() {
		...
		let ident = syn::Path::from_string(custom_func_mut).unwrap_or_else(|_| panic!("Could not find function: {}", custom_func_mut));
	...
	if (!mutable || (mutable && attrs.no_edit)) && attrs.custom_func.is_some() {
		...
		let ident = syn::Path::from_string(custom_func).unwrap_or_else(|_| panic!("Could not find function: {}", custom_func));
		return Some(quote_spanned! { field.span() => {
